...
fn try_bob_taker_maker_commit_to_bob_reveal(
    ...
    attempt_transition_to_bob_reveal(event, runtime, local_params, wallet)
}
...
fn try_bob_funded_to_bob_refund_procedure_signature(
    ...
    match &event.request {
        BusMsg::P2p(PeerMsg::RefundProcedureSignatures(refund_proc)) => {
            ...
            let (spend, view) = aggregate_xmr_spend_view(&remote_params, &local_params);
...
fn try_alice_core_arbitrating_setup_to_alice_arbitrating_lock_final(
    ...
    match event.request {
        ...
        {
            let (spend, view) = aggregate_xmr_spend_view(&local_params, &remote_params);
