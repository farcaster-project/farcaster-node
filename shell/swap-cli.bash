_swap-cli() {
    ...
    case "${cmd}" in
        ...
        swap__cli__progress)
            ...
            ;;
        swap__cli__take)
            opts=" -w -h -V -d -c -v -T -m -x -n  --without-validation --help --version --data-dir --config --verbose --tor-proxy --msg-socket --ctl-socket --chain --electrum-server --monero-daemon --monero-rpc-wallet  <PUBLIC_OFFER> <BITCOIN_ADDRESS> "
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
